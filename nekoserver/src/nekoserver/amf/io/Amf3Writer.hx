...
import nekoserver.amf.EnumPath;
...
class StringCache {
	private var strings: Hash<Int>;
	private var nextIndex: Int;

	public function new() {
		strings = new Hash<Int>();
		nextIndex = 0;
	}
...
*/
class Amf3Writer implements nekoserver.io.Writer {
	 ...
	 */
	private function writeAmf3Object(val: Dynamic, ?writeFields: Dynamic -> nekoserver.io.Writer -> Int -> Void) {
	 ...
	 */
	private function writeAmf3Array(array: Array<Dynamic>, ?writeItem: Dynamic -> nekoserver.io.Writer -> Bool) {
	...
	public function writeArray(val: Array <Dynamic>, writeItem: Dynamic -> nekoserver.io.Writer -> Bool) {
		if (null != val) {
			output.writeByte(0x09);
