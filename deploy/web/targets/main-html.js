...
TestSpriteSheet.main = function() {
	flambe.System.init();
	haxe.Log.trace("Update",{ fileName : "TestSpriteSheet.hx", lineNumber : 30, className : "TestSpriteSheet", methodName : "main"});
	flambe.System.hidden.get_changed().connect(function(hidden,wasHidden) {
		haxe.Log.trace("Hidden is now " + Std.string(hidden),{ fileName : "TestSpriteSheet.hx", lineNumber : 32, className : "TestSpriteSheet", methodName : "main"});
	});
	...
	loader.error.connect(function(message) {
		haxe.Log.trace("Load error: " + message,{ fileName : "TestSpriteSheet.hx", lineNumber : 40, className : "TestSpriteSheet", methodName : "main"});
	});
	loader.progressChanged.connect(function() {
		haxe.Log.trace("Loading progress....... " + loader.get_progress() + " of " + loader.get_total(),{ fileName : "TestSpriteSheet.hx", lineNumber : 43, className : "TestSpriteSheet", methodName : "main"});
	});
}
TestSpriteSheet.onSuccess = function(pack) {
	haxe.Log.trace("Loading complete!",{ fileName : "TestSpriteSheet.hx", lineNumber : 48, className : "TestSpriteSheet", methodName : "onSuccess"});
	flambe.System.root.addChild(new flambe.Entity().add(new flambe.display.FillSprite(3158064,flambe.System.get_stage().get_width(),flambe.System.get_stage().get_height())));
	var container = new flambe.Entity();
	flambe.System.root.addChild(container);
	var ts = new flambe.display.tileSheet.TileSheetHelper();
	var ats = ts.prepareAnimTexture(pack.getFile("remiWalk.json",true));
	var arr = [];
	var target = null;
	container.add(new flambe.display.Sprite());
	var _g = 0;
	while(_g < 10) {
		var ii = _g++;
		...
		$as.centerAnchor();
		tentacle.add($as);
		var sprite = flambe.display.tileSheet.AnimSprite.getFrom(tentacle);
		if(target == null) target = sprite;
		haxe.Log.trace(sprite.getNaturalWidth() * (ii + 1),{ fileName : "TestSpriteSheet.hx", lineNumber : 80, className : "TestSpriteSheet", methodName : "onSuccess", customParams : [sprite.getNaturalHeight() * (ii + 1)]});
		sprite.x.set__(sprite.getNaturalWidth() * (ii + 1));
		sprite.y.set__(100 + ii * 0.01);
		arr.push(sprite);
		container.addChild(tentacle);
	}
	var camera = new flambe.display.camera.GCamera(container,new flambe.math.Rectangle(0,0,5000,5000));
	...
	flambe.System.root.addChild(new flambe.Entity().add(camera));
...
flambe.debug.FpsDisplay.prototype = $extend(flambe.Component.prototype,{
	...
	,onUpdate: function(dt) {
		...
		if(this._fpsTime > 1) {
			...
			var sprite = flambe.display.TextSprite.getFrom(this.owner);
			if(sprite != null) sprite.set_text(text); else flambe.Log.info(text);
			this.reset();
		}
	}
	,get_name: function() {
...
flambe.display.camera = {}
flambe.display.camera.GCamera = function(canvas,bound) {
	this.canvas = canvas;
	this._bound = bound;
};
flambe.display.camera.GCamera.__name__ = true;
flambe.display.camera.GCamera.__super__ = flambe.Component;
flambe.display.camera.GCamera.prototype = $extend(flambe.Component.prototype,{
	to: function(toX,toY,zoomAmount,duration) {
		var xx = toX * zoomAmount;
		var yy = toY * zoomAmount;
		var hx = flambe.System.get_stage().get_width() / 2;
		var hy = flambe.System.get_stage().get_height() / 2;
		if(xx - this._bound.get_left() * zoomAmount < hx) xx = this._bound.get_left() * zoomAmount + hx; else if(this._bound.get_right() * zoomAmount - xx < hx) xx = this._bound.get_right() * zoomAmount - hx;
		if(yy - this._bound.get_top() * zoomAmount < hy) yy = this._bound.get_top() * zoomAmount + hy; else if(this._bound.get_bottom() * zoomAmount - yy < hy) yy = this._bound.get_bottom() * zoomAmount - hy;
		this.tgtx = flambe.System.get_stage().get_width() * 0.5 - xx;
		this.tgty = flambe.System.get_stage().get_height() * 0.5 - yy;
		var child = this.canvas.firstChild;
		while(child != null) {
			var next = child.next;
			var sp = child.firstComponent;
			sp.x.animateBy(this.tgtx,duration);
			sp.y.animateBy(this.tgty,duration);
	...
	,__class__: flambe.display.camera.GCamera
});
...
flambe.display.tileSheet.AnimSprite.getFrom = function(entity) {
	return entity.getComponent("Sprite_1");
}
...
flambe.display.tileSheet.AnimSprite.prototype = $extend(flambe.display.Sprite.prototype,{
	...
	,advanceFrame: function() {
		if(Std["int"](this.curFrame) == this.curAnim.arFrames.length - 1) {
	...
	,play: function(name) {
		...
		if(this.curAnim == null) {
			haxe.Log.trace("play: cannot find sequence: " + name,{ fileName : "AnimSprite.hx", lineNumber : 151, className : "flambe.display.tileSheet.AnimSprite", methodName : "play"});
			return;
		}
...
flambe.scene.Director.getFrom = function(entity) {
	return entity.getComponent("Director_4");
}
...
flambe.scene.Scene.getFrom = function(entity) {
	return entity.getComponent("Scene_5");
}
